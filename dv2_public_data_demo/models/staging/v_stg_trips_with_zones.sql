...
with
    trips as (select * from {{ ref("v_stg_trips") }}),

    zones as (select * from {{ ref("v_stg_zones") }}),
...
#}
    ...
    geo_joined as (
        ...
        from valid_trips as t
        left join
            zones as z
            on st_contains(
                z.zone_geom, st_makepoint(t.pickup_longitude, t.pickup_latitude)
            )
    )
